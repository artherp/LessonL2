//supondo que o idPedido e idTransação sao iguais:
...
public class Mercado {
     ...
     *              correspondentes aos vendedores após a conclusão do pagamento de
     *              um
     *              pedido.
     */
     ...
     *
     * @param pedido             O pedido contendo os produtos e vendedores
     *                           associados.
     ...
     *                             pagamento, como falha na criação do instrumento
     *                             de pagamento.
     */
    public InstrumentoPagamento_if criaInstrumentoPagamentoPedido(Pedido pedido, Comprador comprador,
            TransacaoPagamento_if transacaoPagamento) throws Exception {
        double total = pedido.calcularTotal();
        System.out.println("Mercado está processando o pagamento de R$ " + total);

        try {
            InstrumentoPagamento_if instrumento = transacaoPagamento.criaInstrumentoPagamento(pedido.getId(), total,
                    comprador.getConta(), conta);
            return instrumento;
        } catch (Exception e) {
            throw e;
        }
    ...
    private void onNewDeposit(ListenEvent event) {
        ...
        for (Pedido pedido : pedidos) {
            if (pedido.getId() == depositEvent.idTransacao) {
                for (ItemPedido item : pedido.getItens()) {
                    ...
                    try {
                        ...
                        InstrumentoPagamento_if qrCode = pix.criaInstrumentoPagamento(pedido.getId(), valorDeposito,
                                conta, vendedorConta);
...
/*
 * supondo que idTransacao e idPedido sao diferentes:
 * 
 * import java.util.ArrayList;
 ...
 * 
 * public InstrumentoPagamento_if criaInstrumentoPagamentoPedido(Pedido pedido,
 * Comprador comprador, TransacaoPagamento_if transacaoPagamento) throws
 * Exception {
 * double total = pedido.calcularTotal();
 * System.out.println("Mercado está processando o pagamento de R$ " + total);
 * 
 * try {
 * InstrumentoPagamento_if instrumento =
 * transacaoPagamento.criaInstrumentoPagamento(pedido.getId(), total,
 * comprador.getConta(), conta);
 * return instrumento;
 * } catch (Exception e) {
 * throw e;
 ...
 * 
 * ListenTransactionEvent depositEvent = (ListenTransactionEvent) event;
 * List<Pedido> pedidosCorrespondentesPorValor =
 * pedidosPorValor.get(depositEvent.valor);
 ...
 * try {
 * PagamentoPix pix = new PagamentoPix();
 * InstrumentoPagamento_if qrCode = pix.criaInstrumentoPagamento(pedido.getId(),
 * valorDeposito, conta, vendedorConta);
 * conta.pagarInstrumento(qrCode);
 * } catch (Exception ex) {}
 * }
